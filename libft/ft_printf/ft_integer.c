...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/06/17 14:26:14 by jsiller           #+#    #+#             */
/*   Updated: 2021/07/01 15:54:04 by jsiller          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_precise(char *str)
{
	...
	if (g_precision < ft_strlen_b(str))
		ret = malloc(g_width + ft_strlen_b(str) + 1);
	else
...
int	ft_integer(int big)
{
	...
	if (big == -1)
		str = ft_itoa(va_arg(g_args, int));
	else
		str = ft_hex_buf(va_arg(g_args, unsigned int), big);
	if (!str)
		return (-1);
	if (g_precision < -1)
		g_precision = -1;
